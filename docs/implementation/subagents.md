...
- 组件测试通过后，必须立即更新 `apps/playground` 展示页；未同步 showcase 不算完成。
- 本地 showcase 只认 `apps/playground`，不要再引用或维护根目录 `src/`。
- **遇到不理解、不合理、需求冲突：必须先问用户，不允许猜。**
- **每次输出“下一阶段计划”时必须达到详细标准**（目标边界、文件级产物、接口契约、测试矩阵、子代理任务卡、门禁、风险提问项），否则不允许进入实现。
...
- 已更新 `apps/playground` 展示页，确保最新完成的组件在示例中可见。
